...
class Parser:
    ...
    """
    cmd: a string for a commandline which will be executed to retrieve metrics
    filename: a string for the name of a file where metric data are stored
    ttpye: text type - block, json, yaml, xml
    scheduler: scheduler type - SLURMJOB is the default
    """
    def __init__(self, cmd, filename="", ttype="block", scheduler=""):
        ...
        self._ttype = ttype
        # imported from the constant module
        self._maps = const.KEYWORDS[scheduler]
        self._keys = self._maps.values()
        self._records = {}
        # a temporary file to store output from executing self._cmd
        self._tmp_filename = "/tmp/" + str(uuid.uuid4())

    """
    Run cmd and store its output in the file whose name is specified in self._tmp_filename
    """
    def _run_cmd(self):
        if self._cmd:
    ...
    """
    delete the tmp file that stores the output from executing cmd
    """
    ...
    def get_records(self):
        ...
        for ml in mylist:
            ...
            if any("RUNNING" in s for s in ml):
                ...
                groupid = self._process_userid(job_record["groupid"])
                jobid = job_record["jobid"]
                self._records[jobid] = {}
                self._records[jobid]["userid"] = userid
                self._records[jobid]["groupid"] = groupid
                ...
                self._records[jobid]["start_ts"] = datetime.fromisoformat(job_record["starttime"]).timestamp()
